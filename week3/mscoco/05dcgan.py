...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, batchsize, shuffle=True):
            ...
            train_err += np.array(train_fn(noise, targets))
            train_batches += 1
        ...
        X = X_test[0:64]
        y = gen_fn(lasagne.utils.floatX(np.random.rand(64, 100)))
        X[:, :, 16:48, 16:48] = y[:, :, :, :]
        im = Image.fromarray((X.reshape(8, 8, 3, 64, 64).transpose(1, 3, 0, 4, 2) * 255).astype('uint8').reshape(8*64, 8*64, 3))
        im.save("05dcgan.jpeg")


    # Save model
    np.savez('05dcgan_gen.npz', *lasagne.layers.get_all_param_values(generator))
    np.savez('05dcgan_disc.npz', *lasagne.layers.get_all_param_values(discriminator))
